...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        $.getJSON("/order/list?orderstatus=2", function (data) {
            jQuery(grid_selector).jqGrid({
                data: data,
                datatype: "local",
                height: "100%",
                colNames: ['操作', '商品','订单号','订单状态','单价','数量','付款金额','预定人','电话', '地址','下单时间','支付时间'],
                colModel: [
                    {name: 'myac', index: 'myac', width: 60,align:'center', fixed: true, search:false,sortable: false, resize: false,formatter:'myaction'},
                    {name: 'goodName', index: 'goodName',align:'center',search:false},
                    {name: 'ordersn', index: 'ordersn',align:'center'},
                    {name: 'goodstatus', index: 'goodstatus',align:'center',editable: true,edittype:"select",search:false,formatter:orderstatus,editoptions:{value:"1:发货"}},
                    ...
                    {name: 'paytime', index: 'paytime',align:'center',search:false,formatter:'date', formatoptions:{srcformat:'u', newformat: 'Y-m-d H:i'}},
                ...
                caption: "已消费<i class='icon-double-angle-right'></i>",
                autowidth: true
            });

            function orderstatus (cellvalue, options, rowObject) {
                if(cellvalue == 0 ){
                    return "<span class='label label-sm label-info'>未发货</span>";
                }else if(cellvalue == 1){
                    return "<span class='label label-sm label-danger'>已发货</span>";
                }else if(cellvalue == 2){
                    return "<span class='label label-sm label-success'>已收货</span>";
                }else if(cellvalue == 3){
                    return "<span class='label label-sm label-default'>已评论</span>";
                }else{
                    return "无记录";
                }
            }
